...
func (s *PlaybookRunService) AddParticipants(ctx context.Context, playbookRunID string, userIDs []string, forceAddToChannel bool) error {
	request := struct {
		...
		UserIDs: userIDs,
		Force:   forceAddToChannel,
	}

	req, err := s.client.newAPIRequest(http.MethodPost, fmt.Sprintf("runs/%s/participants", playbookRunID), request)
	if err != nil {
		return err
	}
...
func (s *PlaybookRunService) RemoveParticipants(ctx context.Context, playbookRunID string, userIDs []string, requesterID string) error {
	request := struct {
		UserIDs []string `json:"user_ids"`
	}{
		UserIDs: userIDs,
	...
	req, err := s.client.newAPIRequest(http.MethodPost, fmt.Sprintf("runs/%s/participants/remove", playbookRunID), request)
	if err != nil {
		return fmt.Errorf("failed to create request to remove participants: %w", err)
	}
