...
func (s *PlaybookRunService) Archive(ctx context.Context, playbookRunID string) error {
	archiveUrl := fmt.Sprintf("runs/%s/archive", playbookRunID)
	req, err := s.client.newRequest(http.MethodDelete, archiveUrl, nil)
	if err != nil {
		return err
	}
