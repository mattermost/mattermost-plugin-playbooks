#!/usr/bin/env bash
# seed.sh — Populates a Mattermost instance with playbooks and runs that exercise
# the sequential/unique run identifier features in this branch.
#
# Features seeded:
#   1. Sequential run IDs (run_number_prefix on playbook, run_number + sequential_id on runs)
#   2. Channel name template ({SEQ}, {Zone}, {Attack Type} tokens)
#   3. Required attributes — template-referenced fields are required at run creation
#   4. /playbook run command opens the modal (not a server-side dialog)
#
# Usage:
...
MM_URL="${POSITIONAL[0]:-${MM_URL:-http://localhost:8065}}"
...
db_cleanup() {
    ...
    local stmts=(
        "DELETE FROM ir_run_participants  WHERE IncidentID IN (SELECT ID FROM ir_incident WHERE TeamID = '$TEAM_ID')"
        "DELETE FROM ir_statusposts       WHERE IncidentID IN (SELECT ID FROM ir_incident WHERE TeamID = '$TEAM_ID')"
        "DELETE FROM ir_timelineevent     WHERE IncidentID IN (SELECT ID FROM ir_incident WHERE TeamID = '$TEAM_ID')"
        "DELETE FROM ir_metric            WHERE IncidentID IN (SELECT ID FROM ir_incident WHERE TeamID = '$TEAM_ID')"
        "DELETE FROM ir_channelaction     WHERE ChannelID  IN (SELECT ChannelID FROM ir_incident WHERE TeamID = '$TEAM_ID')"
        "DELETE FROM ir_viewedchannel     WHERE ChannelID  IN (SELECT ChannelID FROM ir_incident WHERE TeamID = '$TEAM_ID')"
        "DELETE FROM ir_category_item     WHERE ItemID     IN (SELECT ID FROM ir_incident WHERE TeamID = '$TEAM_ID')"
        "DELETE FROM ir_category_item     WHERE ItemID     IN (SELECT ID FROM ir_playbook WHERE TeamID = '$TEAM_ID')"
        "DELETE FROM ir_incident          WHERE TeamID = '$TEAM_ID'"
        "DELETE FROM ir_condition         WHERE PlaybookID IN (SELECT ID FROM ir_playbook WHERE TeamID = '$TEAM_ID')"
        "DELETE FROM ir_metricconfig      WHERE PlaybookID IN (SELECT ID FROM ir_playbook WHERE TeamID = '$TEAM_ID')"
        "DELETE FROM ir_playbookmember    WHERE PlaybookID IN (SELECT ID FROM ir_playbook WHERE TeamID = '$TEAM_ID')"
        "DELETE FROM ir_playbookautofollow WHERE PlaybookID IN (SELECT ID FROM ir_playbook WHERE TeamID = '$TEAM_ID')"
        "DELETE FROM ir_playbook          WHERE TeamID = '$TEAM_ID'"
    )
    if [[ "$MM_URL" == *"localhost"* || "$MM_URL" == *"127.0.0.1"* ]]; then
        local db="${MM_WORKTREE_DB:-mattermost_test}"
...
PB1_RESP=$(pb_api POST /playbooks -d "{
    ...
    \"description\": \"Playbook with sequential IDs (INC-00001, INC-00002, ...) and a channel name template that uses {SEQ}, {Zone}, and {Attack Type}.\",
    \"team_id\": \"$TEAM_ID\",
    \"public\": true,
...
# Set run_number_prefix — creates the sequential ID series "INC-00001", "INC-00002", ...
echo "    Setting: runNumberPrefix=INC, retrospectiveEnabled=false..."
PB1_JSON=$(pb_api GET "/playbooks/$PB1_ID")
PB1_UPDATED=$(echo "$PB1_JSON" | jq '.run_number_prefix = "INC" | .retrospective_enabled = false')
pb_api PUT "/playbooks/$PB1_ID" -d "$PB1_UPDATED" > /dev/null

# Create property fields BEFORE setting channelNameTemplate.
# Template validation requires the referenced fields to already exist.
echo "    Creating property field: Zone (select)..."
ZONE_FIELD_ID=$(pb_api POST "/playbooks/$PB1_ID/property_fields" -d '{
    "name": "Zone",
    "type": "select",
    "attrs": {
        "options": [
...
echo "    Zone field ID: $ZONE_FIELD_ID"

echo "    Creating property field: Attack Type (select)..."
ATTACK_FIELD_ID=$(pb_api POST "/playbooks/$PB1_ID/property_fields" -d '{
    "name": "Attack Type",
    "type": "select",
    "attrs": {
        "options": [
...
echo "    Attack Type field ID: $ATTACK_FIELD_ID"

# Read back fields to get option IDs for seeding run property values
echo "    Reading back property fields to get option IDs..."
FIELDS_JSON=$(pb_api GET "/playbooks/$PB1_ID/property_fields")

zone_option_id()   { echo "$FIELDS_JSON" | jq -r ".[] | select(.name==\"Zone\") | .attrs.options[] | select(.name==\"$1\") | .id"; }
attack_option_id() { echo "$FIELDS_JSON" | jq -r ".[] | select(.name==\"Attack Type\") | .attrs.options[] | select(.name==\"$1\") | .id"; }

ZONE_ALPHA=$(zone_option_id "Alpha")
ZONE_BRAVO=$(zone_option_id "Bravo")
ZONE_CHARLIE=$(zone_option_id "Charlie")
ATTACK_RANSOM=$(attack_option_id "Ransomware")
ATTACK_PHISH=$(attack_option_id "Phishing")
ATTACK_DDOS=$(attack_option_id "DDoS")

for _var_name in ZONE_ALPHA ZONE_BRAVO ZONE_CHARLIE ATTACK_RANSOM ATTACK_PHISH ATTACK_DDOS; do
    _val="${!_var_name}"
    if [[ -z "$_val" ]]; then
        echo "ERROR: could not resolve option ID for $_var_name — check .data.name in property fields response" >&2
        echo "Fields JSON: $FIELDS_JSON" >&2
        exit 1
    fi
done
echo "    Option IDs resolved."

# Set channelNameTemplate — {SEQ} expands to the sequential ID (e.g. INC-00001),
# {Zone} and {Attack Type} refer to the property fields created above and are
# automatically marked required at run creation because they appear in the template.
echo "    Setting: channelNameTemplate={SEQ} - {Zone} - {Attack Type}..."
PB1_JSON=$(pb_api GET "/playbooks/$PB1_ID")
PB1_UPDATED=$(echo "$PB1_JSON" | jq '.channel_name_template = "{SEQ} - {Zone} - {Attack Type}"')
pb_api PUT "/playbooks/$PB1_ID" -d "$PB1_UPDATED" > /dev/null

# --- Create 3 runs for Playbook 1 (INC-00001, INC-00002, INC-00003) ---
create_pb1_run() {
    local zone_val="$1" attack_val="$2" owner_val="$3"

    local payload
    payload=$(jq -n \
        --arg name "__template__" \
        --arg owner "$owner_val" \
        --arg team "$TEAM_ID" \
        --arg pb "$PB1_ID" \
        --arg zf "$ZONE_FIELD_ID" --arg zv "$zone_val" \
        --arg af "$ATTACK_FIELD_ID" --arg av "$attack_val" \
        '{name: $name, owner_user_id: $owner, team_id: $team, playbook_id: $pb,
          property_values: {($zf): $zv, ($af): $av}}')

    local run_json
    run_json=$(pb_api POST /runs -d "$payload")

    local run_id seq_id run_name
    run_id=$(echo "$run_json" | jq -r '.id // empty')
    seq_id=$(echo "$run_json" | jq -r '.sequential_id // empty')
    run_name=$(echo "$run_json" | jq -r '.name // empty')
    if [ -z "$run_id" ]; then
        echo "    ERROR creating run. Response: $(echo "$run_json" | jq -c '.' 2>/dev/null || echo "$run_json")" >&2
    fi
    echo "    Run: $run_name (ID: $run_id, SeqID: $seq_id)"
    echo "$run_id"
}

echo ""
echo "--- Creating runs for Playbook 1 (INC-00001, INC-00002, INC-00003)..."
RUN1_ID=$(create_pb1_run "$ZONE_ALPHA"   "$ATTACK_RANSOM" "$OPERATOR_ID" | tail -1)
RUN2_ID=$(create_pb1_run "$ZONE_BRAVO"   "$ATTACK_PHISH"  "$OPERATOR_ID" | tail -1)
RUN3_ID=$(create_pb1_run "$ZONE_CHARLIE" "$ATTACK_DDOS"   "$ADMIN_ID"    | tail -1)

# Add operator as participant to all PB1 runs
echo "--- Adding operator as participant to PB1 runs..."
for RID in $RUN1_ID $RUN2_ID $RUN3_ID; do
...
# --- Verify prefix mutability: prefix can be changed even after runs exist ---
# Existing run IDs are frozen at creation (stored in DB); only new runs get the new prefix.
echo "--- Verifying prefix mutability on PB1 (runs now exist)..."
PB1_JSON=$(pb_api GET "/playbooks/$PB1_ID")
PB1_CHANGED=$(echo "$PB1_JSON" | jq '.run_number_prefix = "CHANGED"')
CHANGE_HTTP=$(pb_api PUT "/playbooks/$PB1_ID" -d "$PB1_CHANGED" -o /dev/null -w "%{http_code}" 2>/dev/null || echo "000")
if echo "$CHANGE_HTTP" | grep -q "^2"; then
    echo "    Prefix change OK: PUT /playbooks/$PB1_ID (runNumberPrefix=CHANGED) succeeded."
else
    echo "    WARNING: Expected success for prefix change after runs exist, got HTTP $CHANGE_HTTP"
fi

# Restore the original prefix
PB1_JSON=$(pb_api GET "/playbooks/$PB1_ID")
PB1_RESTORED=$(echo "$PB1_JSON" | jq '.run_number_prefix = "INC"')
RESTORE_HTTP=$(pb_api PUT "/playbooks/$PB1_ID" -d "$PB1_RESTORED" -o /dev/null -w "%{http_code}" 2>/dev/null || echo "000")
if echo "$RESTORE_HTTP" | grep -q "^2"; then
    echo "    Prefix restore OK: PUT /playbooks/$PB1_ID (runNumberPrefix=INC) succeeded."
else
    echo "    WARNING: Expected success for prefix restore, got HTTP $RESTORE_HTTP"
fi

# --- Verify sequential_id is stored on each run (not computed client-side) ---
echo "--- Verifying sequential_id is stored on PB1 runs..."
for RID in $RUN1_ID $RUN2_ID $RUN3_ID; do
    SEQ=$(curl -sS -X GET "$PB_API/runs/$RID" -H "$(auth_header)" | jq -r '.sequential_id // empty')
    if [ -n "$SEQ" ] && [ "$SEQ" != "null" ]; then
        echo "    Run $RID: sequential_id=$SEQ (stored in DB)"
    else
        echo "    WARNING: Run $RID has no sequential_id stored (expected INC-N)"
    fi
done
...
PB2_ID=$(pb_api POST /playbooks -d "{
    ...
    \"retrospective_reminder_interval_seconds\": 3600,
    \"retrospective_template\": \"Please fill out the retrospective for this release:\",
    \"channel_mode\": \"create_new_channel\",
    \"members\": [
        {\"user_id\": \"$ADMIN_ID\", \"roles\": [\"playbook_admin\", \"playbook_member\"]},
        {\"user_id\": \"$OPERATOR_ID\", \"roles\": [\"playbook_member\"]}
...
echo "=========================================="
echo ""
echo "Playbook 1: Cyber Incident Response"
echo "  ID: $PB1_ID"
echo "  run_number_prefix: INC"
echo "  channel_name_template: {SEQ} - {Zone} - {Attack Type}"
echo "  Required attributes: Zone (select), Attack Type (select)"
echo "  Runs: INC-00001 (Alpha/Ransomware), INC-00002 (Bravo/Phishing), INC-00003 (Charlie/DDoS)"
echo ""
echo "Playbook 2: Release Checklist"
echo "  ID: $PB2_ID"
echo "  run_number_prefix: (none)"
echo "  Runs: 1 run with sequential_id=00001 (plain number, no prefix)"
echo ""
echo "Users:"
...
echo ""
echo "Run IDs:"
echo "  RUN1=$RUN1_ID  (INC-00001)"
echo "  RUN2=$RUN2_ID  (INC-00002)"
echo "  RUN3=$RUN3_ID  (INC-00003)"
echo "  PB2_RUN=$PB2_RUN_ID  (00001)"
echo ""
...
ATTACK_FIELD_ID=$ATTACK_FIELD_ID
PB_API=$PB_API
GQL=$GQL
API=$API
EOF
