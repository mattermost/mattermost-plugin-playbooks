...
func TestApplyAssigneeUpdate(t *testing.T) {
	...
	t.Run("group-assigned item — AssigneeGroupID cleared when user assigned", func(t *testing.T) {
		// Item previously assigned to a group; now assigning to a specific user.
		// applyAssigneeUpdate must clear AssigneeGroupID and set type to specific-user.
		item := &ChecklistItem{
		...
		assert.False(t, noChangeNeeded, "store write must happen: type and group ID changed")
		assert.Equal(t, AssigneeTypeSpecificUser, item.AssigneeType)
		assert.Equal(t, "", item.AssigneeGroupID, "AssigneeGroupID must be cleared when user is assigned")
	})

	t.Run("specific-user item with stale group ID — group ID cleared", func(t *testing.T) {
		// Edge-case: item has AssigneeType=specific-user and same AssigneeID but a stale
		// AssigneeGroupID left over from a previous group assignment. noChangeNeeded must
		// be false so the stale AssigneeGroupID gets persisted as cleared.
		item := &ChecklistItem{
			AssigneeType:    AssigneeTypeSpecificUser,
			AssigneeGroupID: "stale-group",
			AssigneeID:      "user-1",
		}

		noChangeNeeded := applyAssigneeUpdate(item, "user-1")

		assert.False(t, noChangeNeeded, "store write must happen to clear stale AssigneeGroupID")
		assert.Equal(t, "", item.AssigneeGroupID, "stale AssigneeGroupID must be cleared")
	})
}

// TestApplyGroupAssigneeUpdate tests the applyGroupAssigneeUpdate helper.
func TestApplyGroupAssigneeUpdate(t *testing.T) {
	t.Run("unassigned item — group assigned, store write required", func(t *testing.T) {
		item := &ChecklistItem{
			AssigneeType:    AssigneeTypeSpecificUser,
			AssigneeID:      "",
			AssigneeGroupID: "",
		...
		noChangeNeeded := applyGroupAssigneeUpdate(item, "group-1")

		assert.False(t, noChangeNeeded, "store write must happen: type and group ID changed")
		assert.Equal(t, AssigneeTypeGroup, item.AssigneeType)
		assert.Equal(t, "group-1", item.AssigneeGroupID)
		assert.Equal(t, "", item.AssigneeID, "AssigneeID must be cleared when group is assigned")
	})

	t.Run("user-assigned item — group replaces user, AssigneeID cleared", func(t *testing.T) {
		item := &ChecklistItem{
			AssigneeType:    AssigneeTypeSpecificUser,
			AssigneeID:      "user-1",
			AssigneeGroupID: "",
		...
		assert.False(t, noChangeNeeded, "store write must happen: type, group and user ID changed")
		assert.Equal(t, AssigneeTypeGroup, item.AssigneeType)
		assert.Equal(t, "group-1", item.AssigneeGroupID)
		assert.Equal(t, "", item.AssigneeID, "AssigneeID must be cleared")
	})

	t.Run("same group already assigned — idempotent, no store write needed", func(t *testing.T) {
		item := &ChecklistItem{
			AssigneeType:    AssigneeTypeGroup,
			AssigneeID:      "",
			AssigneeGroupID: "group-1",
		...
		require.True(t, noChangeNeeded, "no store write needed: already same group")
		assert.Equal(t, AssigneeTypeGroup, item.AssigneeType)
		assert.Equal(t, "group-1", item.AssigneeGroupID)
	})

	t.Run("different group — store write required, group ID updated", func(t *testing.T) {
		item := &ChecklistItem{
			AssigneeType:    AssigneeTypeGroup,
			AssigneeID:      "",
			AssigneeGroupID: "group-1",
		...
		noChangeNeeded := applyGroupAssigneeUpdate(item, "group-2")

		assert.False(t, noChangeNeeded, "store write must happen: group ID changed")
		assert.Equal(t, AssigneeTypeGroup, item.AssigneeType)
		assert.Equal(t, "group-2", item.AssigneeGroupID)
		assert.Equal(t, "", item.AssigneeID)
	})

	t.Run("owner-type item — group overrides role assignment", func(t *testing.T) {
		item := &ChecklistItem{
			AssigneeType:    AssigneeTypeOwner,
			AssigneeID:      "user-1",
			AssigneeGroupID: "",
		...
		assert.False(t, noChangeNeeded, "store write must happen: type changed from owner to group")
		assert.Equal(t, AssigneeTypeGroup, item.AssigneeType)
		assert.Equal(t, "group-1", item.AssigneeGroupID)
		assert.Equal(t, "", item.AssigneeID, "role-resolved AssigneeID cleared when group assigned")
	})
}
