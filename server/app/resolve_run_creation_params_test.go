...
func TestResolveRunCreationParams_UnlockedPropertyTemplateAllowsDialogAndCommandSources(t *testing.T) {
	zoneField := PropertyField{
		PropertyField: mm_model.PropertyField{
			ID:   "fld_zone",
			Name: "Zone",
	...
	for _, source := range []string{RunSourceDialog, RunSourceCommand} {
		t.Run(source, func(t *testing.T) {
			run := &PlaybookRun{PlaybookID: pb.ID}
			err := svc.ResolveRunCreationParams(run, pb, nil, source)

			require.NoError(t, err, "an unlocked property-based template is only a prefill and is never resolved via %s, so it must not block run creation from that source", source)
		})
	}
}

func TestResolveRunCreationParams_LockedPropertyTemplateBlocksDialogAndCommandSources(t *testing.T) {
	zoneField := PropertyField{
		PropertyField: mm_model.PropertyField{
			ID:   "fld_zone",
			Name: "Zone",
	...
	for _, source := range []string{RunSourceDialog, RunSourceCommand} {
		t.Run(source, func(t *testing.T) {
			...
			require.Error(t, err, "a locked property-based template will be authoritative, and %s cannot supply the property values it needs", source)
			assert.True(t, errors.Is(err, ErrMalformedPlaybookRun))
		})
	}
}
