...
func (s *PlaybookRunServiceImpl) CreatePlaybookRun(playbookRun *PlaybookRun, pb *Playbook, userID string, public bool, source string, channelDisplayName string, initialPropertyValues map[string]json.RawMessage) (*PlaybookRun, error) {
	...
	// guard prevents future internal callers from bypassing that constraint.
		...
		if err != nil {
			...
			if s.licenseChecker.ConditionalPlaybooksAllowed() {
				...
				if err != nil {
					...
					// Evaluate all conditions to set initial visibility state and collect triggered actions.
					var triggeredActions []ConditionActionDef
					if len(conditionMapping) > 0 {
						...
						if evalErr != nil {
							...
							triggeredActions = evalResult.TriggeredActions
						}
					}
					...
					// Execute triggered actions AFTER persisting — ChangeOwner re-reads from store,
					// so the run must be saved first to avoid UpdatePlaybookRun overwriting the owner.
					if len(triggeredActions) > 0 {
						actorID := playbookRun.OwnerUserID
						if actorID == "" {
							actorID = playbookRun.ReporterUserID
						}
						if actorID != "" {
							s.executeConditionActions(actorID, playbookRun, triggeredActions)
						}
					}
				}
			} else {
...
func (s *PlaybookRunServiceImpl) ChangeOwner(playbookRunID, userID, ownerID string) error {
	...
	//   • AddParticipants above writes to IR_Incident_Members (separate table) — no run-row fields affected.
	//   • resolveOwnerRoleAssignments mutates only ChecklistItem.AssigneeID within checklists.
	//   • UpdatePlaybookRunOwner persists CommanderUserID + ChecklistsJSON, covering all mutations above.
	// If future changes cause AddParticipants or resolveOwnerRoleAssignments to mutate other run
	// fields, those fields must be added to UpdatePlaybookRunOwner or a full UpdatePlaybookRun used instead.
	if err := s.store.UpdatePlaybookRunOwner(playbookRunID, ownerID, func(checklists []Checklist) []Checklist {
		resolveOwnerRoleAssignments(checklists, ownerID)
		return checklists
	}); err != nil {
...
func (s *PlaybookRunServiceImpl) ModifyCheckedState(playbookRunID, userID, newState string, checklistNumber, itemNumber int) error {
	...
	// Enforce per-task completion restriction.
	// Only fetch the playbook (for admin bypass) when at least one checklist item
	// in this run has RestrictCompletionToAssignee set; otherwise the Get call is
	// unnecessary because CanModifyTaskState returns nil immediately.
	{
		var pb *Playbook
		if playbookRunToModify.PlaybookID != "" && anyItemRestrictsCompletion(playbookRunToModify.Checklists) {
			if playbook, pbErr := s.playbookService.Get(playbookRunToModify.PlaybookID); pbErr == nil {
				pb = &playbook
			} else {
				logrus.WithError(pbErr).WithField("playbook_run_id", playbookRunID).Warn("failed to load playbook for task state check; playbook admin bypass unavailable")
			}
		}
		if lockErr := s.permissions.CanModifyTaskState(userID, itemToCheck, playbookRunToModify, pb); lockErr != nil {
			return lockErr
		}
	}
...
func (s *PlaybookRunServiceImpl) SetPropertyUserAssignee(userID, playbookRunID string, checklistNumber, itemNumber int, propertyFieldID string) error {
	...
	// Validate the field exists and is a User type.
	// Note: GetPropertyField does not scope by playbook; cross-playbook spoofing is
	// prevented because the field ID must be set via the playbook editor (property_user
	// assignee type stores the field ID from the playbook's own field list).
	...
	// Fetch the current property value for the run.
	...
	timestamp := model.GetMillis()
	itemToCheck.AssigneeModified = timestamp
	updateChecklistAndItemTimestamp(&playbookRun.Checklists[checklistNumber], itemToCheck, timestamp)
...
func (s *PlaybookRunServiceImpl) SetRoleAssignee(playbookRunID, userID, assigneeType string, checklistNumber, itemNumber int) error {
	auditRec := plugin.MakeAuditRecord("setChecklistItemRoleAssignee", model.AuditStatusFail)
	defer s.api.LogAuditRec(auditRec)

	model.AddEventParameterToAuditRec(auditRec, "userID", userID)
	model.AddEventParameterToAuditRec(auditRec, "playbookRunID", playbookRunID)
	model.AddEventParameterToAuditRec(auditRec, "assigneeType", assigneeType)
	model.AddEventParameterToAuditRec(auditRec, "checklistNumber", checklistNumber)
	model.AddEventParameterToAuditRec(auditRec, "itemNumber", itemNumber)
...
func (s *PlaybookRunServiceImpl) SkipChecklistItem(playbookRunID, userID string, checklistNumber, itemNumber int) error {
	...
	// unnecessary because CanModifyTaskState returns nil immediately.
	{
		...
		item := playbookRunToModify.Checklists[checklistNumber].Items[itemNumber]
		if lockErr := s.permissions.CanModifyTaskState(userID, item, playbookRunToModify, pb); lockErr != nil {
			return lockErr
		}
	}
...
func (s *PlaybookRunServiceImpl) leaveActions(playbookRun *PlaybookRun, userID string, requesterID string) {
	...
	if err != nil {
		...
		}).Error("leaveActions: failed to get channel")
		return
	}
...
func (s *PlaybookRunServiceImpl) SetRunPropertyValue(userID, playbookRunID, propertyFieldID string, value json.RawMessage) (*PropertyValue, error) {
	...
	if !s.propertyValuesEqual(propertyField, currentValue, value) {
		valueChanged = true
		summary, conditionTransform, err := s.conditionService.EvaluateConditionsForTransform(run, propertyFieldID, currentValue)
		if err != nil {
			return nil, errors.Wrap(err, "failed to evaluate property conditions")
		}
		evaluationResult = summary

		// Apply both transforms in memory so the WS diff reflects the net change.
		resolvePropertyUserAssignmentsForField(run.Checklists, propertyFieldID, newUserID)
		run.Checklists = conditionTransform(run.Checklists)

		// Atomic DB write: SELECT FOR UPDATE reads the freshest checklists from the DB
		// before applying the transforms, eliminating the TOCTOU race that a plain
		// UpdatePlaybookRun(run) would introduce when concurrent checklist mutations
		// occur between GetPlaybookRun and this write.
		atomicTransform := func(freshChecklists []Checklist) []Checklist {
			resolvePropertyUserAssignmentsForField(freshChecklists, propertyFieldID, newUserID)
			return conditionTransform(freshChecklists)
		}
		if err := s.store.UpdatePlaybookRunChecklistsAtomic(playbookRunID, atomicTransform); err != nil {
			return nil, errors.Wrap(err, "failed to atomically update run checklists")
		}
		...
		// Value unchanged — still resolve assignees. Must run unconditionally: new
		// condition tasks added since the last set need resolution even when the value
		...
		if assigneesChanged {
			...
			assigneeTransform := func(freshChecklists []Checklist) []Checklist {
				resolvePropertyUserAssignmentsForField(freshChecklists, propertyFieldID, newUserID)
				return freshChecklists
			}
			if err := s.store.UpdatePlaybookRunChecklistsAtomic(playbookRunID, assigneeTransform); err != nil {
...
// executeConditionActions processes actions triggered by conditions that transitioned to met.
// Actions are system-initiated (triggered by condition rules), so they bypass permission checks
// like OwnerOnlyFinish / RunChangeOwner.
func (s *PlaybookRunServiceImpl) executeConditionActions(userID string, run *PlaybookRun, actions []ConditionActionDef) {
	formatFunc := s.makeRunNameFormatFunc()
	for _, action := range actions {
		switch action.Type {
		case ConditionActionTypeSetOwner:
			// run.OwnerUserID is a snapshot and may be stale by the time ChangeOwner is called.
			// This is safe because ChangeOwner re-reads from the store and has its own idempotency
			// guard (no-op if the new owner already matches the stored owner).
			if action.SetOwnerUserID != "" && action.SetOwnerUserID != run.OwnerUserID {
				if run.TeamID != "" && !IsMemberOfTeam(action.SetOwnerUserID, run.TeamID, s.pluginAPI) {
					logrus.WithFields(logrus.Fields{
						"run_id":       run.ID,
						"new_owner_id": action.SetOwnerUserID,
						"team_id":      run.TeamID,
					}).Warn("condition action: set_owner target is not a member of the run's team; skipping")
					break
				}
				if err := s.ChangeOwner(run.ID, userID, action.SetOwnerUserID); err != nil {
					logrus.WithError(err).WithFields(logrus.Fields{
						"run_id":       run.ID,
						"new_owner_id": action.SetOwnerUserID,
		...
		case ConditionActionTypeNotifyChannel:
			message := action.NotifyMessage
			if message == "" {
				message = "A condition was met on run " + run.Name
			}
			valueMap := make(map[string]json.RawMessage, len(run.PropertyValues))
			for _, pv := range run.PropertyValues {
				valueMap[pv.FieldID] = pv.Value
			}
			systemTokens := s.buildSystemTokens(run, run.SequentialID)
			resolved, _ := ResolveTemplate(message, ResolveOptions{
				Fields:       run.PropertyFields,
				Values:       valueMap,
				SystemTokens: systemTokens,
				FormatFunc:   formatFunc,
			})
			for _, channelID := range action.NotifyChannelIDs {
				if _, err := s.poster.PostMessage(channelID, resolved); err != nil {
					logrus.WithError(err).WithFields(logrus.Fields{
						"run_id":     run.ID,
						"channel_id": channelID,
					}).Warn("condition action: failed to post notification")
				}
			}
		}
	}
...
func (s *PlaybookRunServiceImpl) ResolveRunCreationParams(playbookRun *PlaybookRun, pb *Playbook, initialValues map[string]json.RawMessage, source string) (string, error) {
	...
	// The playbook's ChannelMode is also checked during playbook save (ValidateNewChannelOnlyMode),
	// so by the time we reach run creation, ChannelID != "" is the definitive signal.
	...
	// Evaluate creation rules against initial property values.
	// Creation rules are a conditional-playbooks feature; skip evaluation when the license does not allow it.
	if s.licenseChecker.ConditionalPlaybooksAllowed() && len(pb.CreationRules) > 0 {
		savedPropertyValues := playbookRun.PropertyValues
		evalValues := make([]PropertyValue, 0, len(initialValues))
		for fieldID, rawVal := range initialValues {
			evalValues = append(evalValues, PropertyValue{
				FieldID: fieldID,
				Value:   rawVal,
			})
		}
		playbookRun.PropertyValues = evalValues

		preOwner := playbookRun.OwnerUserID
		evaluateCreationRules(pb.CreationRules, playbookRun)
		playbookRun.PropertyValues = savedPropertyValues

		if playbookRun.OwnerUserID != preOwner {
			logrus.WithFields(logrus.Fields{
				"playbook_id":    pb.ID,
				"new_owner_id":   playbookRun.OwnerUserID,
				"prior_owner_id": preOwner,
			}).Info("owner assigned by creation rule")
		}

		// Prevent creation-rule channel assignment from violating NewChannelOnly.
		if pb.NewChannelOnly && playbookRun.ChannelID != "" {
			return "", errors.Wrap(ErrMalformedPlaybookRun, "creation rule attempts to link an existing channel, but this playbook requires new channels only")
		}
		// NOTE: creation rules are evaluated on ALL source paths including RunSourceDialog and
		// RunSourceCommand. SetChannelID injection from a creation rule is intentional on those
		// paths — the channel assignment is pre-authorised by the playbook admin who configured
		// the rule. The createPlaybookRun caller re-validates the injected channel ID for existence
		// and caller access (playbook_runs.go: "creation rule: verify channel exists and is accessible"),
		// providing the runtime authorization gate.
	}
...
// resolveRoleAssignments resolves BOTH Owner and Creator role assignments at run creation time.
// Called once during CreatePlaybookRun, after checklists are copied from the playbook template.
// Returns a new []Checklist so the caller's backing array is not mutated.
...
// applyPropertyUserAssigneeUpdate sets the item's assignee type to property_user, sets the field
// ID and the already-resolved user ID. Returns true if no change was needed (all fields already match).
func applyPropertyUserAssigneeUpdate(item *ChecklistItem, propertyFieldID string, resolvedUserID string) (noChangeNeeded bool) {
	noChangeNeeded = propertyFieldID == item.AssigneePropertyFieldID &&
...
// extractUserIDFromPropertyValue extracts a user ID from a JSON property value.
// Property values for User fields are stored as quoted JSON strings like `"uXYZ123"`.
func extractUserIDFromPropertyValue(value json.RawMessage) string {
	if value == nil {
		return ""
	}
	var userID string
...
func resolvePropertyUserAssignmentsForField(checklists []Checklist, fieldID, userID string) bool {
	...
	for ci := range checklists {
		for ii := range checklists[ci].Items {
			item := &checklists[ci].Items[ii]
			// AssigneePropertyFieldID equality is the load-bearing guard; the AssigneeType
			// check prevents accidentally retyping items that happen to share the field ID.
			if item.AssigneeType == AssigneeTypePropertyUser && item.AssigneePropertyFieldID == fieldID {
...
// resolvePropertyUserAssignmentsFromRun resolves property_user assignees by looking up
// each item's field ID in the run's in-memory PropertyValues slice.
// Returns true if any item's AssigneeID changed.
func resolvePropertyUserAssignmentsFromRun(run *PlaybookRun) bool {
	if run == nil {
		return false
	}
	...
	for ci := range run.Checklists {
		for ii := range run.Checklists[ci].Items {
			...
			if item.AssigneeType != AssigneeTypePropertyUser || item.AssigneePropertyFieldID == "" {
				continue
			}
...
// remapAssigneePropertyFieldIDs updates AssigneePropertyFieldID on checklist items
// from playbook-level field IDs to run-level field IDs using the provided mapping.
func remapAssigneePropertyFieldIDs(checklists []Checklist, fieldMappings map[string]string) {
	for ci := range checklists {
		for ii := range checklists[ci].Items {
			item := &checklists[ci].Items[ii]
			if item.AssigneePropertyFieldID == "" {
				continue
			}
...
// anyItemRestrictsCompletion returns true if any checklist item in the run has
// RestrictCompletionToAssignee set to true. This is used to gate the more
// expensive playbookService.Get() call that enables the playbook-admin bypass
// in CanModifyTaskState: if no item carries the restriction the bypass
// path can never be reached, so the Get is unnecessary.
func anyItemRestrictsCompletion(checklists []Checklist) bool {
