...
func NewActionsHandler(router *mux.Router, channelActionsService app.ChannelActionService, pluginAPI *pluginapi.Client, permissions *app.PermissionsService) *ActionsHandler {
	...
	channelActionsRouter.HandleFunc("", withLogger(handler.createChannelAction)).Methods(http.MethodPost)
	channelActionsRouter.HandleFunc("", withLogger(handler.getChannelActions)).Methods(http.MethodGet)
	channelActionsRouter.HandleFunc("/check-and-send-message-on-join", withLogger(handler.checkAndSendMessageOnJoin)).Methods(http.MethodGet)

	channelActionRouter := channelActionsRouter.PathPrefix("/{action_id:[A-Za-z0-9]+}").Subrouter()
	channelActionRouter.HandleFunc("", withLogger(handler.updateChannelAction)).Methods(http.MethodPut)

	return handler
}

func (a *ActionsHandler) createChannelAction(w http.ResponseWriter, r *http.Request, logger logrus.FieldLogger) {
	userID := r.Header.Get("Mattermost-User-ID")

	vars := mux.Vars(r)
	channelID := vars["channel_id"]

	if !a.PermissionsCheck(w, logger, a.permissions.ChannelActionCreate(userID, channelID)) {
		return
	}

	var channelAction app.GenericChannelAction
	if err := json.NewDecoder(r.Body).Decode(&channelAction); err != nil {
		a.HandleErrorWithCode(w, logger, http.StatusBadRequest, "unable to parse action", err)
		return
	}
	...
	if channelAction.ChannelID != channelID {
		a.HandleErrorWithCode(w, logger, http.StatusBadRequest, "channel ID in request body must match channel ID in URL", nil)
		return
	}

	// Validate the action type and payload
	if err := a.channelActionsService.Validate(channelAction); err != nil {
		a.HandleErrorWithCode(w, logger, http.StatusBadRequest, "invalid action", err)
		return
	}

	id, err := a.channelActionsService.Create(channelAction)
	if err != nil {
		a.HandleErrorWithCode(w, logger, http.StatusInternalServerError, "unable to create action", err)
		return
	}
...
func (a *ActionsHandler) getChannelActions(w http.ResponseWriter, r *http.Request, logger logrus.FieldLogger) {
	userID := r.Header.Get("Mattermost-User-ID")

	vars := mux.Vars(r)
	channelID := vars["channel_id"]

	if !a.PermissionsCheck(w, logger, a.permissions.ChannelActionView(userID, channelID)) {
		return
	}

	options, err := parseGetChannelActionsOptions(r.URL.Query())
	if err != nil {
		a.HandleErrorWithCode(w, logger, http.StatusBadRequest, errors.Wrapf(err, "bad options").Error(), err)
	}

	actions, err := a.channelActionsService.GetChannelActions(channelID, *options)
	if err != nil {
		a.HandleErrorWithCode(w, logger, http.StatusInternalServerError, fmt.Sprintf("unable to retrieve actions for channel %s", channelID), err)
		return
	}
...
func (a *ActionsHandler) checkAndSendMessageOnJoin(w http.ResponseWriter, r *http.Request, logger logrus.FieldLogger) {
	vars := mux.Vars(r)
	channelID := vars["channel_id"]
	userID := r.Header.Get("Mattermost-User-ID")
...
func (a *ActionsHandler) updateChannelAction(w http.ResponseWriter, r *http.Request, logger logrus.FieldLogger) {
	userID := r.Header.Get("Mattermost-User-ID")

	vars := mux.Vars(r)
	channelID := vars["channel_id"]

	if !a.PermissionsCheck(w, logger, a.permissions.ChannelActionUpdate(userID, channelID)) {
		return
	}
	...
	if err != nil {
		a.HandleErrorWithCode(w, logger, http.StatusInternalServerError, fmt.Sprintf("unable to update action with ID %q", newChannelAction.ID), err)
		return
	}
