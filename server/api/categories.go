...
func NewCategoryHandler(router *mux.Router, api *pluginapi.Client, categoryService app.CategoryService, playbookService app.PlaybookService, playbookRunService app.PlaybookRunService) *CategoryHandler {
	...
	categoriesRouter.HandleFunc("", withLogger(handler.getMyCategories)).Methods(http.MethodGet)
	categoriesRouter.HandleFunc("", withLogger(handler.createMyCategory)).Methods(http.MethodPost)
	categoriesRouter.HandleFunc("/favorites", withLogger(handler.addFavorite)).Methods(http.MethodPost)
	categoriesRouter.HandleFunc("/favorites", withLogger(handler.deleteFavorite)).Methods(http.MethodDelete)
	categoriesRouter.HandleFunc("/favorites", withLogger(handler.isFavorite)).Methods(http.MethodGet)

	categoryRouter := categoriesRouter.PathPrefix("/{id:[A-Za-z0-9]+}").Subrouter()
	categoryRouter.HandleFunc("", withLogger(handler.updateMyCategory)).Methods(http.MethodPut)
	categoryRouter.HandleFunc("", withLogger(handler.deleteMyCategory)).Methods(http.MethodDelete)
	categoryRouter.HandleFunc("/collapse", withLogger(handler.collapseMyCategory)).Methods(http.MethodPut)

	return handler
}

func (h *CategoryHandler) getMyCategories(w http.ResponseWriter, r *http.Request, logger logrus.FieldLogger) {
	params := r.URL.Query()
...
func (h *CategoryHandler) createMyCategory(w http.ResponseWriter, r *http.Request, logger logrus.FieldLogger) {
	userID := r.Header.Get("Mattermost-User-ID")

	var category app.Category
	if err := json.NewDecoder(r.Body).Decode(&category); err != nil {
		h.HandleErrorWithCode(w, logger, http.StatusBadRequest, "unable to decode category", err)
		return
	}

	if category.ID != "" {
		h.HandleErrorWithCode(w, logger, http.StatusBadRequest, "Category given already has ID", nil)
		return
	}

	// user can only create category for themselves
	if category.UserID != userID {
		h.HandleErrorWithCode(w, logger, http.StatusBadRequest, fmt.Sprintf("userID %s and category userID %s mismatch", userID, category.UserID), nil)
		return
	}
...
func (h *CategoryHandler) updateMyCategory(w http.ResponseWriter, r *http.Request, logger logrus.FieldLogger) {
	vars := mux.Vars(r)
	categoryID := vars["id"]
	userID := r.Header.Get("Mattermost-User-ID")
	...
	if categoryID != category.ID {
		h.HandleErrorWithCode(w, logger, http.StatusBadRequest, "categoryID mismatch in patch and body", nil)
		return
	}

	// user can only update category for themselves
	if category.UserID != userID {
		h.HandleErrorWithCode(w, logger, http.StatusBadRequest, "user ID mismatch in session and category", nil)
		return
	}
	...
	if err != nil {
		h.HandleErrorWithCode(w, logger, http.StatusBadRequest, "Can't get category", err)
		return
	}

	if existingCategory.DeleteAt != 0 {
		h.HandleErrorWithCode(w, logger, http.StatusNotFound, "Category deleted", nil)
		return
	}

	if existingCategory.UserID != category.UserID {
		h.HandleErrorWithCode(w, logger, http.StatusBadRequest, "UserID mismatch", nil)
		return
	}
...
func (h *CategoryHandler) collapseMyCategory(w http.ResponseWriter, r *http.Request, logger logrus.FieldLogger) {
	vars := mux.Vars(r)
	categoryID := vars["id"]
	userID := r.Header.Get("Mattermost-User-ID")
	...
	if err := json.NewDecoder(r.Body).Decode(&collapsed); err != nil {
		h.HandleErrorWithCode(w, logger, http.StatusBadRequest, "unable to decode collapsed", err)
		return
	}
	...
	if existingCategory.UserID != userID {
		h.HandleErrorWithCode(w, logger, http.StatusForbidden, "UserID mismatch", nil)
		return
	}
...
func (h *CategoryHandler) deleteMyCategory(w http.ResponseWriter, r *http.Request, logger logrus.FieldLogger) {
	vars := mux.Vars(r)
	categoryID := vars["id"]
	userID := r.Header.Get("Mattermost-User-ID")
...
func (h *CategoryHandler) addFavorite(w http.ResponseWriter, r *http.Request, logger logrus.FieldLogger) {
	userID := r.Header.Get("Mattermost-User-ID")
	params := r.URL.Query()
	teamID := params.Get("team_id")
	...
	if err := json.NewDecoder(r.Body).Decode(&item); err != nil {
		h.HandleErrorWithCode(w, logger, http.StatusBadRequest, "unable to decode category item", err)
		return
	}
...
func (h *CategoryHandler) deleteFavorite(w http.ResponseWriter, r *http.Request, logger logrus.FieldLogger) {
	userID := r.Header.Get("Mattermost-User-ID")
	params := r.URL.Query()
	teamID := params.Get("team_id")
...
func (h *CategoryHandler) isFavorite(w http.ResponseWriter, r *http.Request, logger logrus.FieldLogger) {
	userID := r.Header.Get("Mattermost-User-ID")
	params := r.URL.Query()
	teamID := params.Get("team_id")
