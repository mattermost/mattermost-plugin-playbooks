...
func NewSignalHandler(router *mux.Router, api *pluginapi.Client, playbookRunService app.PlaybookRunService, playbookService app.PlaybookService, keywordsThreadIgnorer app.KeywordsThreadIgnorer) *SignalHandler {
	...
	keywordsRouter.HandleFunc("/run-playbook", withLogger(handler.playbookRun)).Methods(http.MethodPost)
	keywordsRouter.HandleFunc("/ignore-thread", withLogger(handler.ignoreKeywords)).Methods(http.MethodPost)

	return handler
}

func (h *SignalHandler) playbookRun(w http.ResponseWriter, r *http.Request, logger logrus.FieldLogger) {
	publicErrorMessage := "unable to decode post action integration request"

	var req *model.PostActionIntegrationRequest
	err := json.NewDecoder(r.Body).Decode(&req)
	if err != nil {
		h.returnError(publicErrorMessage, err, logger, w)
		return
	}
	if req == nil {
		h.returnError(publicErrorMessage, errors.New("nil request"), logger, w)
		return
	}
	...
	if err != nil {
		h.returnError(fmt.Sprintf("unable to get original post with ID %q", postID), err, logger, w)
		return
	}

	pbook, err := h.playbookService.Get(id)
	if err != nil {
		h.returnError("can't get chosen playbook", errors.Wrapf(err, "can't get chosen playbook, id - %s", id), logger, w)
		return
	}
	...
	if err := h.playbookRunService.OpenCreatePlaybookRunDialog(req.TeamId, req.UserId, req.TriggerId, postID, "", []app.Playbook{pbook}, isMobile, post.Id); err != nil {
		h.returnError("can't open dialog", errors.Wrap(err, "can't open a dialog"), logger, w)
		return
	}
...
func (h *SignalHandler) ignoreKeywords(w http.ResponseWriter, r *http.Request, logger logrus.FieldLogger) {
	publicErrorMessage := "unable to decode post action integration request"
	...
	if err := h.api.Post.DeletePost(req.PostId); err != nil {
		h.returnError("unable to delete original post", err, logger, w)
		return
	}
}
