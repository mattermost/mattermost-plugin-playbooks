...
func (r *RunRootResolver) AddRunParticipants(ctx context.Context, args struct {
	...
	// Group expansion is a manage-participants operation, so gate requests before
	// resolving any group membership from caller-supplied IDs.
	if len(args.GroupIDs) > 0 || !updatesOnlyRequesterMembership(userID, args.UserIDs) {
