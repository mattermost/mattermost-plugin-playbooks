...
func TestTrialLicences(t *testing.T) {
	...
	t.Run("request trial license without permissions", func(t *testing.T) {
		...
		resp, err := doPluginRequest(e.ServerClient, context.Background(), "POST", e.ServerClient.URL+"/plugins/"+manifest.Id+"/api/v0/bot/notify-admins/button-start-trial", string(dialogRequestBytes), nil)
		assert.Error(t, err)
	...
	t.Run("request trial license with permissions", func(t *testing.T) {
		...
		dialogRequestBytes, _ := json.Marshal(dialogRequest)
		resp, err := doPluginRequest(e.ServerAdminClient, context.Background(), "POST", e.ServerClient.URL+"/plugins/"+manifest.Id+"/api/v0/bot/notify-admins/button-start-trial", string(dialogRequestBytes), nil)
