...
const helpText = "###### Mattermost Incident Management Plugin - Slash Command Help\n" +
	...
	"* `/incident status` - Update the incident's status and (optionally) broadcast the status update to another channel. \n" +
...
func getCommand() *model.Command {
	return &model.Command{
		...
		AutoComplete:     true,
		AutoCompleteDesc: "Available commands: start, end, status, stage, restart, check, announce, list, commander, info",
...
func getAutocompleteData() *model.AutocompleteData {
	slashIncident := model.NewAutocompleteData("incident", "[command]",
		"Available commands: start, end, status, restart, check, announce, list, commander, info, stage")
	...
	slashIncident.AddCommand(status)
...
func (r *Runner) actionStatus() {
	...
	if !permissions.CanPostToChannel(r.args.UserId, r.args.ChannelId, r.pluginAPI) {
		r.postCommandResponse("user does not have permissions to post in this channel")
	}
