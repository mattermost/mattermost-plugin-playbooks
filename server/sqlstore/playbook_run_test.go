...
func TestSortBySequentialID(t *testing.T) {
	db := setupTestDB(t)
	store := setupSQLStore(t, db)
	playbookRunStore := setupPlaybookRunStore(t, db)
	setupChannelsTable(t, db)
	...
	run1 := NewBuilder(t).WithName("Run 1").WithTeamID(teamID).ToPlaybookRun()
	run1.RunNumber = 1
	run1.SequentialID = "INC-00001"
	run1, err := playbookRunStore.CreatePlaybookRun(run1)
	require.NoError(t, err)
	createPlaybookRunChannel(t, store, run1)

	run2 := NewBuilder(t).WithName("Run 2").WithTeamID(teamID).ToPlaybookRun()
	run2.RunNumber = 2
	run2.SequentialID = "INC-00002"
	run2, err = playbookRunStore.CreatePlaybookRun(run2)
	require.NoError(t, err)
	createPlaybookRunChannel(t, store, run2)

	run10 := NewBuilder(t).WithName("Run 10").WithTeamID(teamID).ToPlaybookRun()
	run10.RunNumber = 10
	run10.SequentialID = "INC-00010"
	run10, err = playbookRunStore.CreatePlaybookRun(run10)
	require.NoError(t, err)
	createPlaybookRunChannel(t, store, run10)
	...
	t.Run("ascending", func(t *testing.T) {
		results, err := playbookRunStore.GetPlaybookRuns(requesterInfo, app.PlaybookRunFilterOptions{
			TeamID:    teamID,
			Sort:      app.SortBySequentialID,
			Direction: app.DirectionAsc,
			Page:      0,
			PerPage:   10,
		})
		...
		require.Equal(t, "INC-00001", results.Items[0].SequentialID)
		require.Equal(t, "INC-00002", results.Items[1].SequentialID)
		require.Equal(t, "INC-00010", results.Items[2].SequentialID)
	})
	...
	t.Run("descending", func(t *testing.T) {
		...
		require.NoError(t, err)
		require.Len(t, results.Items, 3)
		require.Equal(t, "INC-00010", results.Items[0].SequentialID)
		require.Equal(t, "INC-00002", results.Items[1].SequentialID)
		require.Equal(t, "INC-00001", results.Items[2].SequentialID)
	})
}
