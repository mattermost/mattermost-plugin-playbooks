...
describe('timeline', () => {
    ...
    before(() => {
        cy.apiInitSetup({createIncident: true}).then(({team, channel, user, playbook, incident}) => {
            ...
            //     testUser2 = user2;
                // cy.apiAddUserToTeam(team.id, user2.id);
                // cy.log('INCIDENT CHANNEL ID:  ', incident.channel_id);
                // cy.apiAddUserToChannel(incident.channel_id, otherUser.id);
            // });
        });
    });
    ...
    beforeEach(() => {
        ...
        cy.visit(`/${teamName}/channels/${testIncident.name}`);
    ...
    it.only('timeline updates show the incident created, status updated, commander changed, and checklist events', () => {
        // * Verify incident created message is visible in the timeline
        verifyTimelineEvent('incident_created', 1, 0, `Incident Reported by ${testUsername}`);
        ...
        // verifyTimelineEvent('status_updated', 1, 0, `${testUsername} posted a status update`);

        // // # Change commander
        // cy.executeSlashCommand(`/incident commander @${otherUser.username}`);

        // // * Verify we can see the change commander in the timeline
        // verifyTimelineEvent('commander_changed', 1, 0, `Commander changed from ${testUsername} to @${otherUser.username}`);

        // // # Post an update that changes the incident status
        // cy.updateStatus('this is a status update', 0, 'Active');

        // // * Verify we can see the update in the timeline
        // verifyTimelineEvent('status_updated', 2, 1, `${testUsername} changed status from Reported to Active`);

        // // # Change commander
        // cy.executeSlashCommand(`/incident commander @${testUsername}`);

        // // * Verify we can see the change commander in the timeline
        // verifyTimelineEvent(`commander_changed', 2, 1, 'Commander changed from @${otherUser.username} to ${testUsername}`);
        ...
        // verifyTimelineEvent('task_state_modified', 1, 1, '${testUsername} checked off checklist item "Step 1"');
    });

    it('events from posts in the incident channel show up at the end and in the middle of the timeline', () => {
    ...
    it('events from posts in another channel show up at the end and in the middle of the timeline', () => {
        const summary1 = 'This is the incident summary 1';
        const summary2 = 'This is the incident summary 2';
    ...
    it('timeline updates can be deleted (both standard events and events from posts)', () => {
        // * Verify incident created message is visible in the timeline
    ...
    it('timeline notice shows when there are no events', () => {
        // * See all events:
        clickOnFilterOption('All events');
    ...
    it('timeline filter shows each type, and all', () => {
        // # Post an update that doesn't change the incident status
        cy.updateStatus('this is a status update');
