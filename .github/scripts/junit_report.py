...
def build_summary(total, passed, failed, skipped, annotations):
    ...
    lines += ["", "_Generated by the Playbooks JUnit report publisher._"]
    return "\n".join(lines)


def validate_env():
    required = ["GH_TOKEN", "GITHUB_SHA", "GITHUB_REPOSITORY", "GITHUB_REPOSITORY_OWNER"]
    missing = [v for v in required if not os.environ.get(v)]
    if missing:
        print(f"Error: missing required CI variables: {', '.join(missing)}", file=sys.stderr)
        sys.exit(1)
