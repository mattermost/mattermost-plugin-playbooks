...
describe('playbooks > list', {testIsolation: true}, () => {
    ...
    it('can duplicate playbook with attributes and conditions', () => {
        ...
        // # Capture the original playbook's ID from the DOM (API-returned ID may differ from rendered ID)
        cy.contains('[data-testid="playbook-item"]', 'Playbook with Attributes').then(($item) => {
            originalPlaybookId = $item.attr('data-playbook-id');
        });

        // # Click on the dot menu of the exact original playbook using its ID (fresh query to avoid stale ref)
        cy.then(() => {
        ...
        cy.findAllByTestId('property-field-row').eq(0).within(() => {
            cy.findByLabelText('Attribute name').type('{selectall}Modified Name');
        });
