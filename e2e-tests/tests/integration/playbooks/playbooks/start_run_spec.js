...
describe('playbooks > start a run', {testIsolation: true}, () => {
    ...
    describe('smoke > new_channel_only toggle', () => {
        it('displays the new_channel_only toggle for playbook admins', () => {
            ...
            cy.findByTestId('new-channel-only-toggle').should('be.visible');
        });

        it('toggle state persists after update', () => {
            // # Navigate to the playbook editor
            cy.visit(`/playbooks/playbooks/${testPlaybook.id}/outline`);
            ...
            // # Wait for the confirmation dialog to appear and confirm
            ...
            cy.findByTestId('new-channel-only-toggle').should('be.checked');
        });

        it('run modal enforces new channel when toggle is enabled', () => {
            // # Enable new_channel_only via API
            cy.apiPatchPlaybook(testPlaybook.id, {new_channel_only: true});

            // # Visit the playbook and open run modal
            cy.visit(`/playbooks/playbooks/${testPlaybook.id}/outline`);
            cy.wait(500);
            cy.findByTestId('run-playbook').click();
            ...
            cy.get('#root-portal.modal-open').within(() => {
                ...
                // * Verify the "link existing channel" radio is disabled
                cy.findByTestId('link-existing-channel-radio').should('be.disabled');

                // * Verify the "create new channel" radio is checked and active
                cy.findByTestId('create-channel-radio').should('be.checked');

                // * Verify the channel selector is hidden
                cy.findByTestId('channel-selector').should('not.exist');

                // * Verify enforcement hint is shown
                cy.get('body').should('contain', 'requires a new channel');
            });
        });
    });
});
